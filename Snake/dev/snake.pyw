	...
	while running:
	    for event in pygame.event.get():
	        if event.type == QUIT:
	            ...
	            # Check if the mouse is hovering over a player name
	            for i, player in enumerate(players):
	                text_surface = font.render(f"{player[0]} - Highscore: {player[1]}", True, font_color)
	                text_rect = text_surface.get_rect()
	                text_rect.topleft = (100, 100 + (i * 50))
	                if text_rect.collidepoint(mouse_x, mouse_y):
	                    selected_index = i
	                    break
	        elif event.type == MOUSEBUTTONDOWN:
	            # Check if the mouse clicked on a player name
	            if event.button == 1:
		...
		for event in pygame.event.get():
			...
			if event.type == pygame.MOUSEBUTTONDOWN:
				...
				for pressed in coordinates:
					if pressed['name'] == 'plus':
						input1 = InputBox(500, 200, 50, 20)
					else:
